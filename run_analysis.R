## This script demonstrates the collection and processing of a data set with
## the objective of forming a 'Tidy' data set as per Wickham 2004
## see README.md for a detailed explanation of this script.
...
# Load required files.  The normalizePath() function is used to try to ensure
# that the code can be run on any platform.  The test platform was Windows.
...
# Also add an additional column to indicate if the data is taken from the
# train or the test data.
data.set <- "test"
...
# First identify the required features from feature.txt
feature.required <- grep("mean\\(|std\\(",features$FeatureName)

# Subset the full data set to retain the ... and the required features
...
# Since the activity names are stored in activity_names.txt in the order
# corresponding to their respective ID, we can use the row number of the 
# data frame activity_labels as a substitute for the activity ID. We can
# therefore simply identify the label corresponding to the required
# activity ID using activity_label$ActivityName[<required_ActivityID>].
# In order to apply this operation across the full column of the data,
# we use sapply and an anonymous function to perform the lookup.
...
# Names for the features are taken from the original file features.txt,
# extracting the required values using the vector obtained using the grep() function in step 2.
...
# Two minor adjustments are made to the feature names to make them compatible
# with the eventual output format: parentheses are removed and dashes are
# replaced with dots:
...
# The final data set has 180 rows, one for each combination of the 30 subjects
# and 6 activities.  In addition to the 2 columns identifying the activity and
# subject, are 66 columns providing the average of each variable.
